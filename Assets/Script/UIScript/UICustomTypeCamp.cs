...
public class UICustomTypeCamp : MonoBehaviour
{
    ...
    private void OnStartButtonClicked()
    {
        ...
        {
            Debug.LogWarning("No duration options defined.");
            return;
        }

        if (selectedDurationIndex < 0 || selectedDurationIndex >= durationOptions.Length)
        {
            Debug.LogWarning($"Invalid duration index: {selectedDurationIndex}");
            return;
        }
